...
public class ShopTriggerTA : MonoBehaviour
{
    ...
    public static ShopTriggerTA Instance;
    ...
    {
        // Nếu là player local thì mới cho phép
        if (other.CompareTag("Player"))
        {
            // Ở game online: Nếu muốn chỉ cho player local thì kiểm tra HasInputAuthority ở đây
            isPlayerInZone = true;
    ...
    void Update()
    {
        ...
        if (ShopTriggerVK.Instance.shopPanel != null && ShopTriggerVK.Instance.shopPanel.activeSelf)
            return;
        if (CanvasShop.Instante.canvasDaily != null && CanvasShop.Instante.canvasDaily.activeSelf)
            return;
